 ...
 * [chunk=<I>chunk</I>] [tracker=<I>host</I>[:<I>port</I>]]
 * [listen=<I>host</I>[:<I>port</I>]] [debug=<I>message</I>[,<I>message</I>...]]
 * [jar=<I>file</I>] [timelimit=<I>timelimit</I>] <I>jobOrTaskClass</I>
 * [<I>arg</I> ...]</TT>
 * <P>
 ...
 * for loop. The default is one thread for each core of the machine on which the
 * task is running. See the {@link edu.rit.pj2.Job#threads(Integer)
 * Job.threads()} or {@link edu.rit.pj2.Task#threads(int) Task.threads()} method
 ...
 * partitioned among the threads executing the parallel for loop. The default is
 * a chunk of size 1. See the {@link edu.rit.pj2.Job#chunk(Integer) Job.chunk()}
 * or {@link edu.rit.pj2.Task#chunk(int) Task.chunk()} method for further
 * information.
 * <P><LI>
...
// Exported operations.
	...
	public static void main
		...
		try
			...
			String tracker = null;
			String listen = null;
			EnumSet<Debug> debug = null;
			String jar = null;
			Integer timeLimit = null;
			String jobOrTaskClass = null;
			int argi = 0;
			while (argi < args.length && jobOrTaskClass == null)
				...
				if (args[argi].startsWith ("threads="))
					try
						{
						threads = new Integer (args[argi].substring (8));
						if (threads < 1)
			...
			String[] taskArgs = new String [numTaskArgs];
			...
			if (Task.class.isAssignableFrom (jtclass))
				...
				// Set up a task specification.
				final TaskSpec taskSpec = new TaskSpec (jtclass);
				taskSpec.args (taskArgs);
				if (threads != null)
					taskSpec.requires (new Node() .cores (threads));
				else
					taskSpec.requires (new Node() .cores (Node.ALL_CORES));
...
// Hidden operations.
	...
	private static void usage
		...
		System.err.println ("Usage: java pj2 [threads=<threads>] [schedule=<schedule>] [chunk=<chunk>] [tracker=<host>[:<port>]] [listen=<host>[:<port>]] [debug=<message> ...] [jar=<file>] <jobOrTaskClass> [<arg> ...]");
		System.exit (1);
		}
