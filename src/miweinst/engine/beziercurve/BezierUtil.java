...
public class BezierUtil {
	...
	public static class ClosedBezierSpline
	{
		...
		/// length as the <paramref name="knots"> array.</param>
		public static void GetCurveControlPoints(Vec2f[] knots, 
		 Vec2f[] firstControlPoints, Vec2f[] secondControlPoints)
		{
			int n = knots.length;
			if (n <= 2)
			{ // There should be at least 3 knots to draw closed curve.
				firstControlPoints = new Vec2f[0];
				secondControlPoints = new Vec2f[0];
				return;
			}
			...
			double[] x = cyclicSolve(a, b, c, 1, 1, rhs);
			...
			double[] y = cyclicSolve(a, b, c, 1, 1, rhs);

			// Fill output arrays.
			firstControlPoints = new Vec2f[n];
			secondControlPoints = new Vec2f[n];
			for (int i = 0; i < n; ++i)
			{
				// First control point.
				firstControlPoints[i] = new Vec2f((float)x[i], (float)y[i]);
				// Second control point.
				secondControlPoints[i] = new Vec2f
	...
	/// a and b in the corners.
		...
		/// <returns>The solution vector of size n.</returns>
	    	public static double[] cyclicSolve(double[] a, double[] b, 
