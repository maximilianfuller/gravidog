...
public class CubicBezierCurve extends BezierCurve {
	 ...
	 *
	 * Right now NOT doing it mathematically!! 
	 * Just looping through #res points on curve incrementing t by res each iteration 
	 * to find where MP and dP/dt are closest to 0 (i.e. orthogonal)*/
	public Vec2f nearestPointOnCurve(Vec2f m) {
		//Minimum distance between M and P, i.e. M.minus(P).dot(derivative(getT(P))) == 0
		float res = 1000;
		float t_val = 0;
		
///TRY TO USE root finding instead of iterating
...
/*		float minDist = Float.POSITIVE_INFINITY;
		for (float t=0; t<=1; t=t+1/res) {
			Vec2f p = getCasteljauPoint(t);
			 //This just minimizes distance, rather than orthogonal equation; probably no difference
			if (m.dist2(p) < minDist) {
				minDist = m.dist2(p);
				t_val = t;
			} 
		}
		//Check on either side of t_val from t-1/res to t+1/res by smaller resolution step
		for (float t=t_val-1/res; t<=t_val+1/res; t=t+1/(res*5)) {
			//still on curve
			if (t <= 1 && t >= 0) {
				Vec2f p = getCasteljauPoint(t);
				if (m.dist2(p) < minDist) {
...
//		console.log(t[0]+" "+t[1]+" "+t[2]);
	...
	public boolean collidesCircle(CircleShape c) {	
		Vec2f on = this.nearestPointOnCurve(c.getCenter());
		float dist = on.dist(c.getCenter());
		Vec2f mtv = on.minus(c.getCenter()).normalized().smult(c.getRadius()-dist);
		if (dist <= c.getRadius()) {
