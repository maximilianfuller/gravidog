...
//	private SensorEntity _playerSensor;
...
//////^^^^
	...
	public MWorld(App app, Viewport viewport) {
		...
		//Math coordinates -- origin in lower left
		//Transform matrix: [[X scale, X shear, Y shear], [Y scale, X translate, Y translate]...]
//		_tx = new AffineTransform(super.getScale(), 0, 0, -super.getScale(), super.getPixelGameLocation().x, super.getPixelGameLocation().y+_windowDim.y);
//		super.setTransform(_tx);
...
//////////	
	/*Called on win or lose conditions.*/
...
//		long nanos = nanosSincePreviousTick/super.getIterations();
		for (int i=1; i<=super.getIterations(); i++) {
			...
			if (_arrowKeyStates[1]) {
				if (_player.isStable()) {
					if (_jumping == false) {
						_player.applyImpulse(new Vec2f(0, 45));
						_jumping = true;
					}
				}
			}
...
//			float dx = _player.getLocation().x - prevLoc.x;
//			float dy = _player.getLocation().y - prevLoc.y;
//			super._viewport.pan(dx, super._viewport.isMathCoordinateSystem()? -dy: dy);
...
/////////
/*		PhysicsEntity[] testarr = _testVisualizer.toArr();
		super.setEntityArr(testarr);
		super.draw(g);*/
///////////^^^^
	}
	...
	public void onKeyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == 32) {
			if (_currMouseLoc != null) {
				...
				if (firstHit != null) 
					firstHit.x.applyImpulse(firstHit.y.minus(src));
