...
public class TestCollisionVisualizer {
	...
	private TestEntity _testCircle, _testRect, _testPolygonA, _testPolygonB;
...
//////	TESTER FOR ONLY CHECKING ONE SHAPE AGAINST THE OTHER
		/*if (_testArr[2].collides(_testArr[1])) {
			_testArr[2].setShapeColor(new Color(r,g,b));
			_testArr[1].setShapeColor(new Color(r,g,b));
		}*/
////^^^	
		...
		for (int i=0; i<_testArr.length; i++) {
			for (int j=0; j<_testArr.length; j++) {
				if (_testArr[i] != _testArr[j]) {
					//If Shape has not already been checked against all other shapes
//					if (_testArr[j].isVisited() == false) {
...
//			_testArr[i].setVisited(true);
		}
//		for (int i=0; i<_testArr.length; i++) _testArr[i].setVisited(false);
...
//			test_shape = compound;
			...
			verts[0] = new Vec2f(15, 10);
			verts[1] = new Vec2f(11, 5);
			verts[2] = new Vec2f(11, 15);
